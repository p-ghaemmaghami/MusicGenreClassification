...
model = fitcsvm(squeeze(MultimediaFt),targets,'Standardize',true,'BoxConstraint',1,'Leaveout','on');
model = fitcdiscr(squeeze(MultimediaFt),targets,'Leaveout','on','Gamma',1);
[label,NegLoss,PBScore,Posterior] = kfoldPredict(model);
eval = Evaluate(targets,label);
MCA_ACC = eval(1)
MCA_f1 = eval(6) 
...
%         testTargets = targets(j);
%         model = fitcsvm(trainFeatures,trainTargets,'Standardize',true,'BoxConstraint',1);
%         %model = fitcsvm(trainFeatures,trainTargets,'OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('AcquisitionFunctionName','expected-improvement-plus'));
%         %NaiveBayes.fit(trainFeatures,trainTargets);
...
%     end
    ...
    %model = fitcsvm(squeeze(features(i,:,:)),targets,'Standardize',true,'BoxConstraint',1,'Leaveout','on','OptimizeHyperparameters',{'BoxConstraint'});
    model = fitcsvm(squeeze(features(i,:,:)),targets,'Standardize',true,'BoxConstraint',1,'Leaveout','on');
    %model = fitcdiscr(squeeze(features(i,:,:)),targets,'Leaveout','on');
    [labelMEG(i,:),~,~,~] = kfoldPredict(model);

    eval = Evaluate(targets,labelMEG(i,:)');
